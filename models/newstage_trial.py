...
class TempScaler(torch.nn.Module):
    def __init__(self, init_T=1.0):
        super().__init__()
        self.logT = torch.nn.Parameter(torch.log(torch.tensor([init_T], device=device)))
    def T(self): return torch.exp(self.logT)

def fit_temperature(model, val_loader, device):
    model.eval()
    scaler = TempScaler(1.0).to(device)
    opt = torch.optim.LBFGS([scaler.logT], lr=0.25, max_iter=50)
    bce = nn.BCEWithLogitsLoss(reduction='mean')

    logits_list, targets_list = [], []
    with torch.no_grad():
        for x, y in val_loader:
            x = x.to(device); y = y.unsqueeze(1).float().to(device)
    ...
    def closure():
        opt.zero_grad()
        loss = bce(logits / scaler.T(), targets)
        loss.backward()
        return loss
    opt.step(closure)
...
def dice_metric_from_probs(probs, targets, thr=0.5, eps=1e-7):
    # probs: [B,1,H,W] in [0,1], targets: [B,1,H,W] in {0,1}
    preds = (probs > thr).float()
    inter = (preds * targets).sum(dim=(1,2,3))
    union = preds.sum(dim=(1,2,3)) + targets.sum(dim=(1,2,3))
    dice = torch.where(union==0, torch.ones_like(union), (2*inter)/(union + eps))
    return dice.mean()

# Loss (replace your current)

def criterion(logits, targets):
    return 0.5 * dice(logits, targets) + 0.5 * bce(logits, targets)
...
def process_crop_batch(crops, model, device, transform, transform_name='base', T=1.0):
    batch_tensors = []
    for crop in crops:
    ...
    with torch.no_grad():
        logits = model(batch_tensor)                    # logits
        if transform_name != 'base':
            logits = reverse_tta_predictions(logits, transform_name)
        predictions = torch.sigmoid(logits / T)        # <-- softened probs
...
def predict_large_image_memory_efficient(image, model, device, crop_size=256, stride=128,
...
def validate_model(model, val_loader, criterion, device, *,
    ...
    """
    Validation: evaluates with a unified Dice metric.
    - criterion: should take LOGITS (e.g., 0.5*Dice(from_logits=True)+0.5*BCEWithLogits)
    - temperature: post-hoc calibration T (>1 softens); probs = sigmoid(logits / T)
    - threshold: binarization threshold for the Dice metric
    """
    model.eval()
    val_loss = 0.0
    dice_batches = []
    ...
    with torch.no_grad():
        for images, masks in val_loader:
            ...
            masks  = masks.to(device, non_blocking=True).unsqueeze(1).float()  # [B,1,H,W]

            # Forward -> LOGITS (do NOT sigmoid here)
            logits = model(images)
            ...
            probs = torch.sigmoid(logits / temperature)  # [B,1,H,W]

            # Binarize
            preds = (probs > threshold).float()

            # Dice per-sample with empty-empty = 1.0
            inter = (preds * masks).sum(dim=(1, 2, 3))
            psum  = preds.sum(dim=(1, 2, 3))
            gsum  = masks.sum(dim=(1, 2, 3))
            union = psum + gsum

            dice_per = torch.where(
                union == 0,
                torch.ones_like(union),                       # empty-empty → perfect
                (2.0 * inter) / (union + 1e-7)
            )
            dice_batches.append(dice_per.mean().item())

    avg_val_loss = val_loss / max(num_batches, 1)
    avg_dice = float(np.mean(dice_batches)) if dice_batches else 0.0
...
def find_best_threshold(model, val_loader, device, T=1.0):
    model.eval()
    thrs = torch.linspace(0.2, 0.8, 25, device=device)
    sums = torch.zeros_like(thrs)
    with torch.no_grad():
        for x, y in val_loader:
            x = x.to(device)
            y = y.unsqueeze(1).float().to(device)
            probs = torch.sigmoid(model(x) / T)
            for i, t in enumerate(thrs):
                preds = (probs > t).float()
                inter = (preds*y).sum(dim=(1,2,3))
                union = preds.sum(dim=(1,2,3)) + y.sum(dim=(1,2,3))
                dice = torch.where(union==0, torch.ones_like(union),
                                   (2*inter)/(union+1e-7))
                sums[i] += dice.mean()
    best_i = torch.argmax(sums).item()
    return float(thrs[best_i].item())
# Create full dataset
...
pos_w = 2.0  # try 2–5 if positives are rare
bce = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_w], device=device), label_smoothing=0.05)
...
for epoch in range(num_epochs):
    ...
    for images, masks in train_loader:
        ...
        # Forward pass
        logits = model(images)                     # keep LOGITS here
        loss = criterion(logits, masks)            # <-- new
        ...
        with torch.no_grad():
            ...
            train_dice_sum += dice_metric_from_probs(probs, masks).item()
    ...
    # Validation phase
    avg_val_loss, avg_val_dice = validate_model(model, val_loader, criterion, device,
                                            threshold=0.5, temperature=1)
...
best_thr = find_best_threshold(model, val_loader, device, T)
...
with torch.no_grad():
    ...
    crop_size=256, stride=128, batch_size=4, threshold=best_thr,
    use_tta=False, use_gaussian_weights=True,
    compute_crop_dice=True, ground_truth_mask=pil_mask,
    # add this:
    # (thread through predict_large_image -> process_crop_batch)
)
        ...
        # Save prediction
        save_image(pred_tensor, f"predictions_newstage/pred_{i}_{img_name}")
...
print(f"Model saved as 'best_model_ging.pth'")
