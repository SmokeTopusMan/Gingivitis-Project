...
def predict_large_image(image, model, device, crop_size=1024, stride=512,
...
def predict_large_image_memory_efficient(image, model, device, crop_size=1024, stride=512,
...
# Create test dataset
test_dataset = SegmentationDataset("dataset/test", "dataset/test_mask", transform=val_transform)
...
with torch.no_grad():
    for i, (image, mask) in enumerate(test_loader):
        print(f"\nProcessing test image {i+1}/{len(test_loader)}")

        # Convert tensor to PIL image for prediction function
        image_tensor = image.squeeze(0)  # [3, H, W]
        ...
        if image_area > 2000 * 2000:  # Large image
            print("Using memory-efficient prediction for large image")
            pred = predict_large_image(
                pil_image, model, device,
                crop_size=1024, stride=512, threshold=0.5, max_memory_gb=4
            )
        else:
            print("Using improved prediction with TTA")
            # Use improved prediction with TTA for better accuracy on smaller images
            pred = predict_large_image(
                pil_image, model, device,
                crop_size=1024, stride=512, batch_size=4, threshold=0.5,
        ...
        # Prepare ground truth mask - ensure it's on the same device
        mask = mask.to(device).unsqueeze(1).float()  # [1, 1, H, W]
        pred_tensor = pred_tensor.to(device)  # Move prediction to GPU for comparison
