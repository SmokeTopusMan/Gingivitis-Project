...
def split_mask_if_row_bridge(white_mask: np.ndarray,
    ...
    cum = np.cumsum(m.view(np.uint8), axis=0)
    tot = cum[-1, :]
...
def middle_band_by_row_test(white_mask: np.ndarray,
                            min_width_frac: float = 0.07,
                            min_between_frac: float = 0.45,
                            min_band_rows: int = 5,
...
def hull_concavity_scores(white_mask, comp, band_frac=0.35, min_gap_h_frac=0.10):
    _, gaps = hull_hollow_masks(white_mask)
...
def cut_to_the_chase_efficient(image_path, mask_path):
    ...
    white_mask = binarize_mask(mask, use_otsu=True)
    ...
    if len(components) == 1:
        ...
        if False:
            ...
            pixels_to_black_out = ~inner_hollow | extra_black
        else:
    ...
    white_mask, comp, band_frac=0.35, min_gap_h_frac=0.10
            ...
            tau_area = max(50, int(0.002 * comp.xspan() * comp.yspan()))
            close=0.6666
            picked =None
